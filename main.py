...
def welcomeScreen():
    ...
    while True:
        for event in pygame.event.get():
            #if user click on cross button then close then game
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()

            #if the user presses spacebar or up key then start the game
...
def getRandomPipe():
    """
    generate positions of 2 pipes (one bottom straight and one top rotated) to blit on the screen
    """
    pipeHeight = GAME_SPRITES['pipe'][0].get_height()
...
if __name__ == "__main__":
    # the main function from where game will start
    pygame.init() #initializing all pygame modules
    FPSCLOCK = pygame.time.Clock() #for controlling the game fps
    pygame.display.set_caption('Flappy Bird by TanmayGupta')
